...
def generate_data(n_samples, g_x_true=10.0, alpha_x_true=13.0, beta_x_true=0.7):
    ...
    random_noise = torch.randn(n_samples).view(-1, 1)
    x = F.conv1d(random_noise.unsqueeze(0).unsqueeze(0), torch.ones(1, 1, 5) / 5, padding=2).squeeze(0).squeeze(0)
    x.requires_grad_(True)
...
train(model, optimizer, epochs=20000)
